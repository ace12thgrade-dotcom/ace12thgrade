...
export function getPEContent(chapter: string, type: 'notes' | 'pyqs'): string {
  ...
  if (type === 'notes') {
    const part1 = getPEPart1Notes(lower);
    if (part1) return part1;

    const part2 = getPEPart2Notes(lower);
    if (part2) return part2;

    // Fallback comprehensive revision notes
    return getPEPart2Notes('revision') || `TOPIC: Physical Education Master Notes: ${chapter}
Key concepts, physiological mechanisms, biomechanical principles, and board exam blueprints strictly aligned with CBSE Class 12 NCERT curriculum.`;
    ...
    const pyqs1 = getPEPart1PYQs(lower);
    if (pyqs1) return pyqs1;

    const pyqs2 = getPEPart2PYQs(lower);
    if (pyqs2) return pyqs2;

    // Fallback comprehensive PYQ set
    return getPEPart2PYQs('revision') || `QUESTION: Q1. [3 Marks] Explain the PRICER protocol for soft tissue injuries.
SOLUTION:
