...
export const FormulaCard: React.FC<FormulaCardProps> = ({ formula, theme = 'paper' }) => {
  ...
  return (
    <div className={`my-4 rounded-2xl border transition-all duration-200 overflow-hidden shadow-xs hover:shadow-md ${
      isDark 
      ...
      <div className={`px-4 py-2.5 flex items-center justify-between gap-3 border-b ${
        ...
        <div className="flex items-center gap-2 min-w-0">
          <span className="w-6 h-6 rounded-lg bg-amber-500/20 text-amber-700 dark:text-amber-300 flex items-center justify-center text-xs font-black shrink-0">
            ⚡
          </span>
          <h4 className="text-xs sm:text-sm font-black tracking-tight uppercase truncate">
        ...
        <button
          onClick={handleCopy}
          title="Copy Formula"
          className={`flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-lg transition-all active:scale-95 shrink-0 ${
            copied
      ...
      <div className={`p-4 sm:p-5 flex items-center justify-center text-center ${
        ...
        <div className="w-full">
          <div className="text-base sm:text-xl lg:text-2xl font-mono font-black tracking-wide leading-relaxed px-3 py-2.5 rounded-xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-amber-900 dark:text-amber-300 select-all overflow-x-auto">
            {formula.equation}
          </div>
        </div>
      </div>
      ...
      {formula.variables && formula.variables.length > 0 && (
        <div className="px-4 sm:px-5 py-3 border-t border-inherit bg-black/2 dark:bg-white/2">
          <div className="text-[11px] font-black uppercase tracking-wider mb-2 text-amber-800 dark:text-amber-400 flex items-center gap-1.5">
      ...
      {/* When & Why to Apply */}
      {formula.whenToApply && (
        <div className="px-4 sm:px-5 py-2.5 border-t border-inherit flex items-start gap-2 text-xs">
